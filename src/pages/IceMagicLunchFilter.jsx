// Ice Magic Lunch Filter Page
// Shows clickable section cards when user selects Lunch filter
import React from "react";
import { useParams, useNavigate } from "react-router-dom";
import ItemCard from "../components/ItemCard";
import SectionCard from "../components/SectionCard";
import { IM_LUNCH_SECTIONS, IM_LUNCH_ITEMS } from "../data/menus/iceMagicLunch";

export default function IceMagicLunchFilter() {
    ...
    return (
      <div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <h2 style={{ margin: 0 }} className="page-heading">Ice Magic - Lunch</h2>
        </div>

        <div className="section-list" style={{ marginTop: 12 }}>
          {IM_LUNCH_SECTIONS.map(section => (
            <SectionCard
              key={section.slug}
              english={section.english}
              kannada={section.kannada}
              onClick={() => navigate(`/ice-magic/filter/lunch/${section.slug}`)}
            />
          ))}
        </div>
      </div>
  ...
  const section = IM_LUNCH_SECTIONS.find(s => s.slug === sectionSlug);
  const items = IM_LUNCH_ITEMS[sectionSlug] || [];
    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <button 
          ...
          onClick={() => navigate('/ice-magic/filter/lunch')}
        >
          ← Back
        </button>
