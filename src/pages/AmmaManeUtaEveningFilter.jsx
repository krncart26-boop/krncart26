// Amma Mane Uta Evening Food Filter Page
// Shows clickable section cards when user selects Evening Food section
import React from "react";
import { useParams, useNavigate } from "react-router-dom";
import ItemCard from "../components/ItemCard";
import SectionCard from "../components/SectionCard";
import { AMMA_MANE_UTA_EVENING_SECTIONS, AMMA_MANE_UTA_EVENING_ITEMS } from "../data/menus/ammaManeUtaEvening";

export default function AmmaManeUtaEveningFilter() {
    ...
    return (
      <div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <h2 style={{ margin: 0 }} className="page-heading">Amma Mane Uta - Evening</h2>
        </div>

        <div className="section-list" style={{ marginTop: 12 }}>
          {AMMA_MANE_UTA_EVENING_SECTIONS.map(section => (
            <SectionCard
              key={section.slug}
              english={section.english}
              kannada={section.kannada}
              onClick={() => navigate(`/amma-mane-uta/evening/${section.slug}`)}
            />
          ))}
        </div>
      </div>
  ...
  const section = AMMA_MANE_UTA_EVENING_SECTIONS.find(s => s.slug === sectionSlug);
  const items = AMMA_MANE_UTA_EVENING_ITEMS[sectionSlug] || [];
    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <button 
          ...
          onClick={() => navigate('/amma-mane-uta/evening')}
        >
          ← Back
        </button>
