// Udupi Palace Lunch Filter Page
...
export default function UdupiLunchFilter() {
    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <button 
          ...
          onClick={() => navigate('/udupi-hotel/filter/lunch')}
        >
          ← Back
        </button>
      ...
      <div style={{ padding: '12px' }}>
        {items.length > 0 ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {items.map((item) => {
              // Create unique ID for each lunch item
              const itemId = `udupi-lunch-${sectionSlug}-${item.itemName.toLowerCase().replace(/\s+/g, '-')}`;
              return (
                <ItemCard
                  key={itemId}
                  id={itemId}
